...
public class RisingTides {
    ...
    public int landLost(double height, double newHeight) {
        ...
        return totalOldLand - totalNewLand; // substitute this line. It is provided so that the code compiles.
    }
    ...
    public int numOfIslands(double height) {
        ...
        /* WRITE YOUR CODE BELOW */
		WeightedQuickUnionUF qfu = new WeightedQuickUnionUF(terrain.length, terrain[0].length);
		boolean[][] landGrid = floodedRegionsIn(height);
		...
		for( int rows = 0; rows < terrain.length; rows++ )
		{
			for( int columns = 0; columns < terrain[0].length; columns++ )
			{
				boolean leftCheck = (columns - 1) < terrain[0].length && (columns - 1) >= 0;
				boolean rightCheck = (columns + 1) < terrain[0].length && (columns + 1) >= 0;
				boolean topCheck = (rows - 1) < terrain.length && (rows - 1) >= 0; 
				boolean bottomCheck = (rows + 1) < terrain.length && (rows + 1) >= 0;
				boolean topLeftCheck = (rows - 1) < terrain.length && (rows - 1) >= 0 && (columns - 1) < terrain[0].length && (columns - 1) >= 0;
				boolean topRightCheck = (rows - 1) < terrain.length && (rows - 1) >= 0 && (columns + 1) < terrain[0].length && (columns + 1) >= 0;
				boolean bottomLeftCheck = (rows + 1) < terrain.length && (rows + 1) >= 0 && (columns - 1) < terrain[0].length && (columns - 1) >= 0;
				boolean bottomRightCheck = (rows + 1) < terrain.length && (rows + 1) >= 0 && (columns + 1) < terrain[0].length && (columns + 1) >= 0;

				// if( landGrid[rows + side][columns + side] == false )
				// {
				// 	GridLocation currentCell = new GridLocation(rows, columns);
				// 	GridLocation cellSide = new GridLocation(rows, (columns + side) );
				// 	qfu.union(currentCell, cellSide);
				// }

				if( leftCheck )
				{
					if( landGrid[rows][columns - 1] == false )
					{
						GridLocation currentCell = new GridLocation(rows, columns);
						GridLocation cellSide = new GridLocation(rows, (columns - 1) );
						qfu.union(currentCell, cellSide);
					}
				}
				else if( rightCheck )
				...
				if( topCheck )
				{
					if( landGrid[rows - 1][columns] == false )
					{
						GridLocation currentCell = new GridLocation(rows, columns);
						GridLocation cellSide = new GridLocation( (rows - 1), columns);
						qfu.union(currentCell, cellSide);
					}
				}
				else if( bottomCheck )
				...
				if( topLeftCheck )
				{
					if( landGrid[rows - 1][columns - 1] == false )
					{
						GridLocation currentCell = new GridLocation(rows, columns);
						GridLocation cellSide = new GridLocation( (rows - 1), (columns - 1) );
						qfu.union(currentCell, cellSide);
					}
				}
				else if( topRightCheck )
				{
					if( landGrid[rows - 1][columns + 1] == false )
					{
						GridLocation currentCell = new GridLocation(rows, columns);
		...
		// count islands 
		// 1) iterate through the entire grid through nested for loops 
		// 2) if tile is flooded then skip, else if tile is land check if tile is root node or not  
		// 3) if tile is land and is root node increment islandCount
		// 4) if tile is land and has a root node skip
		// 5) return islandCount
		int islandCount = 0; 
		ArrayList<GridLocation> rootNodes = new ArrayList<GridLocation>();

		for( int rows = 0; rows < terrain.length; rows++ )
		{
			for( int columns = 0; columns < terrain[0].length; columns++ )
			{
				GridLocation cell = new GridLocation(rows, columns);
				GridLocation parentCell = qfu.find(cell);

				if( landGrid[rows][columns] == true )
					continue;
				else if( landGrid[rows][columns] == false )
				{
					if( !rootNodes.contains(parentCell) )
					{
						rootNodes.add(parentCell);
					}
				}
			}
		}

		islandCount = rootNodes.size();

        return islandCount; // substitute this line. It is provided so that the code compiles.
    }
}
