...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        int menuAndCategoryLength = StdIn.readInt();
        menuVar = new MenuNode[menuAndCategoryLength];
        categoryVar = new String[menuAndCategoryLength];
        // WRITE YOUR CODE HERE
        while( StdIn.hasNextLine() )
        {
            for( int i = 0; i < menuAndCategoryLength; i++ )
            {
                StdIn.readLine();
                categoryVar[i] = StdIn.readLine();
                int dishAmount = StdIn.readInt();
                for( int j = 0; j < dishAmount; j++ )
                {
                    StdIn.readLine();
                    String dishName = StdIn.readLine();
                    int[] stockID = new int[StdIn.readInt()];
                    for( int h = 0; h < stockID.length; h++ )
                    {
                        stockID[h] = StdIn.readInt();
                    }
                    Dish newDish = new Dish(categoryVar[i], dishName, stockID);
                    if( menuVar[i] == null )
                    {
                        MenuNode newNode = new MenuNode(newDish, null);
                        menuVar[i] = newNode;
                    }
                    else
                    {
                        MenuNode newNode = new MenuNode(newDish, menuVar[i]);
                        menuVar[i] = newNode;
                    }
                }
            }
    ...
    public void addStockNode ( StockNode newNode ) {
        ...
        // WRITE YOUR CODE HERE
        int hashIndex = newNode.getIngredient().getID() % stockVarSize;
    ...
    public void deleteStockNode ( String ingredientName ) {
        ...
        // WRITE YOUR CODE HERE
        StockNode checkNode = null;
        boolean ingredientFound = false;
        int ingredientLocation = 0;
        // check and make sure that the node exists
        for( int i = 0; i < stockVarSize; i++ )
        {
            checkNode = stockVar[i];
            while( checkNode != null )
            {
                if( checkNode.getIngredient().getName().equals(ingredientName) == true )
                {
                    ingredientFound = true;
                    break;
                }
                else
                    checkNode = checkNode.getNextStockNode();
            }
            if( ingredientFound == true )
            {
                ingredientLocation = i;
                break;
            }
        }
        ...
        StockNode front = stockVar[ingredientLocation];
        if( front.getIngredient().getName().equals(ingredientName) == true )
        {
            stockVar[ingredientLocation] = front.getNextStockNode();
        }
        else
        {
            while( front.getNextStockNode() != null )
            {
                if( front.getNextStockNode().getIngredient().getName().equals(ingredientName) == true )
                {
                    front.setNextStockNode(front.getNextStockNode().getNextStockNode() );
                    break;
                }
                else
                {
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        StockNode checkNode = stockVar[hashIndex];
        do
        {
            if( checkNode.getIngredient().getID() == ingredientID )
                return checkNode;
            else 
                checkNode = checkNode.getNextStockNode();
        }while( checkNode != null );
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        else 
        {
            if( findStockNode(ingredientName) != null )
                findStockNode(ingredientID).getIngredient().updateStockLevel(stockAmountToAdd);
            else if( findStockNode(ingredientID) != null )
                findStockNode(ingredientName).getIngredient().updateStockLevel(stockAmountToAdd);
            else
                return;
        }
    ...
    public void updatePriceAndProfit() {
        ...
        {
            MenuNode target = menuVar[i];
            while( target != null )
            {
                Dish targetDish = target.getDish();
                double cost = 0;
                for( int j = 0; j < targetDish.getStockID().length; j++ )
                {
                    cost += findStockNode(targetDish.getStockID()[j]).getIngredient().getCost();
                }
                double price = cost * 1.2;
                double profit = price - cost;
                targetDish.setPriceOfDish(price);
                targetDish.setProfit(profit);
                target = target.getNextMenuNode();
            }
        }
    }
    ...
    public void createStockHashTable (String inputFile){
        ...
        while( StdIn.hasNextChar() )
        {
            ...
            StdIn.readChar();
            String stockName = StdIn.readLine();
            double stockCost = StdIn.readDouble();
            int stockAmount = StdIn.readInt();

            Ingredient newIngredient = new Ingredient(stockID, stockName, stockAmount, stockCost);
            StockNode newNode = new StockNode(newIngredient, null);
    ...
    public boolean checkDishAvailability ( String dishName, int numberOfDishes ){
        ...
        int[] ingredientsNeededID = foundDish.getStockID();
        for( int i = 0; i < ingredientsNeededID.length; i++ )
        {
            if( findStockNode(ingredientsNeededID[i] ).getIngredient().getStockLevel() < numberOfDishes )
                return false;
            continue;
        }
    ...
    public void order ( String dishName, int quantity ){
        ...
        {
            TransactionData newOrder = new TransactionData( "order", dishName, quantity, findDish(dishName).getDish().getProfit() * quantity, true);
            addTransactionNode(newOrder);
            int[] ingredientIDs = findDish(dishName).getDish().getStockID();
            for( int i = 0; i < ingredientIDs.length; i++ )
            {
                Ingredient ingredient = findStockNode(ingredientIDs[i]).getIngredient();
                updateStock(ingredient.getName(), ingredientIDs[i], quantity * -1);
            }
        }
        else
        {
            TransactionData newOrder = new TransactionData("order", dishName, quantity, 0.00, false);
            addTransactionNode(newOrder);

            MenuNode newDish = findDish(dishName).getNextMenuNode();
            boolean satisfied = false; // if there's a different dish that can be prepped
            while( newDish != findDish(dishName) && satisfied == false )
            {
                if( newDish == null )
                {
                    for( int i = 0; i < categoryVar.length; i++ )
                    {
                        if( categoryVar[i].equals(findDish(dishName).getDish().getCategory() ) )
                        {
                            newDish = menuVar[i];
                        }
                    }
                ...
                if( checkDishAvailability(newDish.getDish().getDishName(), quantity) == true )
                {
                    newOrder = new TransactionData("order", newDish.getDish().getDishName(), quantity, newDish.getDish().getProfit() * quantity, true);
                    satisfied = true;
                    addTransactionNode(newOrder);
                    break;
                }
                newDish = newDish.getNextMenuNode();
            }
            if( satisfied == false )
            {
                if( findDish(dishName).getNextMenuNode() != null )
                {
                    newOrder = new TransactionData("order", findDish(dishName).getNextMenuNode().getDish().getDishName(), quantity, 0.00, false);
                    addTransactionNode(newOrder);
                    return;
                }
                else
                {
                    ...
                    newOrder = new TransactionData("order", newDish.getDish().getDishName(), quantity, 0.00, false);
                    addTransactionNode(newOrder);
                    return;
                }
            }

            int[] ingredientIDs = newDish.getDish().getStockID();
            for( int i = 0; i < ingredientIDs.length; i++ )
            {
                Ingredient ingredient = findStockNode(ingredientIDs[i]).getIngredient();
                updateStock(ingredient.getName(), ingredientIDs[i], quantity * -1);
