...
public class WeatherGenerator {
	...
	public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
		...
		for( int i = 0; i < drywet.length; i++ )
		{
			if( drywet[ i ][ 0] == longitude && drywet[ i ][ 1 ] == latitude )
			{
				for( int j = 0; j < 12; j++ )
				{
					for( int k = 2; k < drywet[ i ].length; k++ )
					{
						drywetProbability[ j ] = drywet[ i ][ k ];
					}
				}
			}
			else
		...
		for( int i = 0; i < wetwet.length; i++ )
		{
			if( wetwet[ i ][ 0] == longitude && wetwet[ i ][ 1 ] == latitude )
			{
				for( int j = 0; j < 12; j++ )
				{
					for( int k = 2; k < wetwet[ i ].length; k++ )
					{
						wetwetProbability[ j ] = wetwet[ i ][ k ];
					}
				}
			}
			else
	...
	public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
		...
		// COMPLETE THIS METHOD
		int[] forecastResult = new int[ numberOfDays ]; 
		double day1 = Math.abs( StdRandom.uniform() );
		for( int i = 0; i < numberOfDays; i++ )
		{
			double tomorrowWeather = Math.abs( StdRandom.uniform() );
			if( day1 < 0.5 )
			{
				if( tomorrowWeather <= wetwetProbability )
				{
					forecastResult[ i ] = WET;
				}
				else
				{
					forecastResult[ i ] = DRY;
				}
			}
			else
			{
				if( tomorrowWeather <= drywetProbability )
				{
					forecastResult[ i ] = WET;
				}
				else
