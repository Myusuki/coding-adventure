#include <raylib.h>
#include <stdlib.h>
#include "tmx_1.10.0-linux/include/tmx.h"
...
int main( int argc, char **argv )
{
  ...
  // set tmx callback funcs; must set these funcs before any loading funcs
  ...
  for( i = 0; i < map->height; i++ )
  {
    for( j = 0; j < map->width; j++ )
    {
      ...
      if( map->tiles[gid] != NULL )
      {
        ...
        draw_tile(image, x, y, w, h, j*tileset->tile_width, i*tileset->tile_height, opacity, flags);
      }
    }
  }
}
...
void draw_polyline( double offset_x, double offset_y, double **points, int points_count, Color color )
{
  ...
  {
    DrawLineEx( (Vector2){offset_x + points[i-1][0], offset_y + points[i-1][1] }, (Vector2){offset_x + points[i][0], offset_y + points[i][1]}, LINE_THICKNESS, color);
  }
}
...
void draw_polygon(double offset_x, double offset_y, double **points, int points_count, Color color)
{
  ...
  if( points_count > 2 )
    DrawLineEx( (Vector2){offset_x + points[0][0], offset_y + points[0][1]}, (Vector2){offset_x + points[points_count-1][0], offset_y + points[points_count-1][1]}, LINE_THICKNESS, color);
}
