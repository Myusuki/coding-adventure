...
int main(void)
{
  ...
  // Load music
  Music music = LoadMusicStream("resources/HPSSChapter1.mp3");
  ...
  PlayMusicStream(music);
	...
	while( !WindowShouldClose() )
	{
				...
				if( IsKeyPressed(KEY_ENTER) )
        {
          PlaySound(fxStart);
			...
			case GAMEPLAY:
			{
        if( IsKeyPressed('P') && gamePaused == false )
            gamePaused = true;
        else if( IsKeyPressed('P') && gamePaused == true )
          gamePaused = false;
    ...
    // music buffers must be refilled when consumed
