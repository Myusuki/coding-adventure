...
int main(void)
{
	...
	for( int rows = 0; rows < BRICKS_LINES; rows++ )
	{
		...
		{
			brick[rows][columns].size = (Vector2){ (float)screenWidth / BRICKS_PER_LINE, 20 };
			brick[rows][columns].position = (Vector2){ columns * brick[rows][columns].size.x, rows * brick[rows][columns].size.y + BRICKS_POSITION_Y };
			brick[rows][columns].bounds = (Rectangle){ brick[rows][columns].position.x, brick[rows][columns].position.y, 
																								brick[rows][columns].size.x, brick[rows][columns].size.y };
			brick[rows][columns].active = true;
		}
	}
