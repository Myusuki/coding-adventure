...
// collision
void UpdateCollision( Player *player, tmx_map *map );
void SetCollisionObjects( tmx_map *map );
...
void UpdatePlayer( tmx_map *map, Camera2D *camera, Player *player, Vector2 screenDim, float deltaTime )
{ 
  Rectangle mapRect = (Rectangle){ 0, 0, map->width * map->tile_width, map->height * map->tile_height };
  // player controls
...
void SetCollisionObjects( tmx_map *map )
{
  ...
  printf( "Amount Collision Objects: %d\n", collisionObjectCount );
}
bool DetectCollision( Player *player, tmx_map *map, int direction )
{
  int xTile = 0, yTile = 0;
